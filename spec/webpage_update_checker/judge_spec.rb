RSpec.describe WebpageUpdateChecker::Judge do
  describe '#current_diff_value' do
    ...
    context 'when the way is SHA256' do
      it 'returns true if uri is valid' do
        obj = WebpageUpdateChecker::Methods::Sha256.new(uri: 'https://www.yahoo.co.jp/')
        expect(checker.current_diff_value(obj)).to be_truthy
      end

      it 'returns Exceotion if uri is invalid' do
        obj = WebpageUpdateChecker::Methods::Sha256.new(uri: 'https://foobar/')
        expect { checker.current_diff_value(obj) }.to raise_error(SocketError)
      end
    end
    ...
    context 'when the way is SHA384' do
      it 'returns true if uri is valid' do
        obj = WebpageUpdateChecker::Methods::Sha384.new(uri: 'https://www.yahoo.co.jp/')
        expect(checker.current_diff_value(obj)).to be_truthy
      end

      it 'returns Exceotion if uri is invalid' do
        obj = WebpageUpdateChecker::Methods::Sha384.new(uri: 'https://foobar/')
        expect { checker.current_diff_value(obj) }.to raise_error(SocketError)
      end
    end
    ...
    context 'when the way is SHA512' do
      it 'returns true if uri is valid' do
        obj = WebpageUpdateChecker::Methods::Sha512.new(uri: 'https://www.yahoo.co.jp/')
        expect(checker.current_diff_value(obj)).to be_truthy
      end

      it 'returns Exceotion if uri is invalid' do
        obj = WebpageUpdateChecker::Methods::Sha512.new(uri: 'https://foobar/')
        expect { checker.current_diff_value(obj) }.to raise_error(SocketError)
      end
    end

    context 'when the way is Last-Modified' do
      # TODO: These are duplicated test codes... (spec/webpage_update_checker/Methods/*)
      it 'returns true if uri is valid and uri has Last-Modified header' do
        obj = WebpageUpdateChecker::Methods::LastModified.new(uri: 'http://www.ne.jp/asahi/pursuits/ootsuki/pc/X68k/x680x0.xhtml')
        expect(checker.current_diff_value(obj)).to be_truthy
      end

      it 'returns nil if uri is valid and uri does not have Last-Modified header' do
        obj = WebpageUpdateChecker::Methods::LastModified.new(uri: 'https://www.aiseikai.or.jp/')
        expect(checker.current_diff_value(obj)).to eq nil
      end

      it 'returns Exceotion if uri is invalid' do
        obj = WebpageUpdateChecker::Methods::LastModified.new(uri: 'https://foobar/')
        expect{ checker.current_diff_value(obj) }.to raise_error(SocketError)
      end
    end
  end
